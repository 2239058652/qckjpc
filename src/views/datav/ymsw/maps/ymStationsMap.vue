<template>
  <div class="datav-layout-container" style="width: 100%">
    <a-layout>
      ...
      <!-- 测试功能使用按钮 -->
      <div style="position: fixed; margin-left: 700px; margin-top: 5px; z-index: 998">
        <a-space>
          <!-- <a-button type="default" danger @click="cesiumFunctionTest(1)"
            >地图测试(开发使用)</a-button
          > -->
        </a-space>
      </div>
      ...
      >
        <!-- <a-tree
          ...
          checkable
          :tree-data="treeData"
          @check="onLayerTreeCheck"
        >
        </a-tree> -->
        <div style="width: 25%">
          <dv-border-box-12 style="padding: 0.5rem 1rem; background-color: rgba(0, 186, 255, 60%)">
            <dv-decoration-11 style="width: 6rem; height: 2rem">水资源</dv-decoration-11>
            <a-checkbox checked="checked">河流</a-checkbox>
            <br />
            <a-checkbox>湖泊</a-checkbox>
            <br />
            <a-checkbox>水库</a-checkbox>
            <br />
            <a-checkbox>坝塘</a-checkbox>
            <br />
            <a-checkbox>湖泊</a-checkbox>
            <br />
            <a-checkbox>沟渠</a-checkbox>
            <br />
          </dv-border-box-12>
          /></div
        >
        <div style="width: 25%">
          <dv-border-box-12 style="padding: 0.5rem 1rem; background-color: rgba(0, 186, 255, 60%)">
            <dv-decoration-11 style="width: 6rem; height: 2rem">监测站点</dv-decoration-11>
            <a-checkbox checked="checked">视频站</a-checkbox>
            <br />
            <a-checkbox>水位站</a-checkbox>
            <br />
            <a-checkbox>雨量站</a-checkbox>
            <br />
            <a-checkbox>流量站</a-checkbox>
            <br />
            <a-checkbox>水质站</a-checkbox>
            <br /> </dv-border-box-12
        ></div>
        <div style="width: 25%">
          <dv-border-box-12 style="padding: 0.5rem 1rem; background-color: rgba(0, 186, 255, 60%)">
            <dv-decoration-11 style="width: 6rem; height: 2rem">工程灌区</dv-decoration-11>
            <a-checkbox>水源工程</a-checkbox>
            <br />
            <a-checkbox>水库除险加固工程</a-checkbox>
            <br />
            <a-checkbox>沟渠工程</a-checkbox>
            <br />
            <a-checkbox>饮水安全工程</a-checkbox>
            <br />
            <a-checkbox>河道治理工程</a-checkbox>
            <br />
            <a-checkbox>高效节水灌区项目</a-checkbox>
            <br />
        ...
        <div style="width: 25%">
          <dv-border-box-12 style="padding: 0.5rem 1rem; background-color: rgba(0, 186, 255, 60%)">
            <dv-decoration-11 style="width: 6rem; height: 2rem">水部门</dv-decoration-11>
            <a-checkbox checked="checked">防汛办</a-checkbox>
            <br />
            <a-checkbox>河长办</a-checkbox>
            <br />
            <a-checkbox>运管站</a-checkbox>
            <br />
            <a-checkbox>灌区局</a-checkbox>
            <br /> </dv-border-box-12
        ></div>
      </div>
      <!-- 地图中鼠标移至显示信息容器 -->
      <!-- 选中对象静态信息显示 -->
      <div
        v-if="mouseMovePickObject.id!"
        style="position: fixed; width: 15%; height: 15%; color: #fff; top: 10%; right: 0px"
      >
        <dv-border-box-12>
          <div class="card-header">
            <div class="card-header-left">ID</div>
            <div class="card-header-right">{{ mouseMovePickObject.id }}</div>
          </div>
          <div class="card-header">
            <div class="card-header-left">类型</div>
            <div class="card-header-right">{{ mouseMovePickObject.type }}</div>
          </div>
          <div class="card-header">
            <div class="card-header-left">名称</div>
            <div class="card-header-right">{{ mouseMovePickObject.name }}</div>
          </div>
          <div class="card-header">
            <div class="card-header-left">内容</div>
            <div class="card-header-right">{{ mouseMovePickObject.content }}</div>
          </div>
        </dv-border-box-12>
      </div>
      <!-- 选中对象动态信息显示 -->
      <div
        v-if="mouseMovePickObject.id!"
        style="position: fixed; width: 30%; height: 50%; color: #fff; bottom: 20%; right: 0px"
      >
        <dv-border-box-11 title="监测信息" class="datav-border-box-content">
          <gradientEchartDemo v-if="mouseMovePickObject.type == 'water'" />
          <sectionsEchartDemo v-else-if="mouseMovePickObject.type == 'rain'" />
          <mutipleEchartsDemo v-else-if="mouseMovePickObject.type == 'video'" />
          <axisEchartsDemo v-else-if="mouseMovePickObject.type == 'city-xz'" />
          <scatterEchartsDemo v-else />
        </dv-border-box-11>
      </div>
      <!-- 经纬度显示 -->
      <div style="position: fixed; height: auto; width: auto; color: #fff; bottom: 0px; right: 0px">
        <!-- <dv-border-box-12> -->
        <div class="card-header">
          <div class="card-header-left">经度：</div>
          <div class="card-header-right">{{ currentMousePosition.longitude }}</div>
        </div>
        <div class="card-header">
          <div class="card-header-left">纬度：</div>
          <div class="card-header-right">{{ currentMousePosition.latitude }}</div>
        </div>
        <div class="card-header">
          <div class="card-header-left">相机高度：</div>
          <div class="card-header-right">{{ currentMousePosition.altitude }}km</div>
        </div>
        <div class="card-header">
          <div class="card-header-left">地图层级：</div>
          <div class="card-header-right">{{ currentMousePosition.zoom }}级</div>
        </div>
        <!-- </dv-border-box-12> -->
      </div>
    </a-layout>
...
<script lang="ts">
  ...
  //echart图表demo，用于显示在监测数据中
  import mutipleEchartsDemo from '../demos/funnel-mutipleEchartsDemo.vue';
  import gradientEchartDemo from '../demos/line-gradientEchartDemo.vue';
  import sectionsEchartDemo from '../demos/line-sectionsEchartDemo.vue';
  import axisEchartsDemo from '../demos/multiple-y-axisEchartsDemo.vue';
  import scatterEchartsDemo from '../demos/scatterEchartsDemo.vue';
  ...
  */
      ...
      const setCurrentMousePosition = (position) => {
        ...
        currentMousePosition.zoom = position.zoom;
        // console.log('currentMousePosition ' + JSON.stringify(currentMousePosition));
      };

      //鼠标移动选中对象
      let mouseMovePickObject = reactive({
        id: '',
        name: '',
        content: '',
        type: '',
      });
      const setMouseMovePickObject = (obj) => {
        if (obj! && obj.id!) {
          mouseMovePickObject.id = obj.id;
          mouseMovePickObject.name = obj.name;
          mouseMovePickObject.content = obj.content;
          mouseMovePickObject.type = obj.type;
        } else {
          mouseMovePickObject.id = '';
          mouseMovePickObject.name = '';
          mouseMovePickObject.content = '';
          mouseMovePickObject.type = '';
        }

        //根据对象id获取需要显示的静态信息和动态信息
      };
      ...
      const initCesiumViewer = () => {
        console.log('ymStationsMap.vue initCesiumViewer');
        //设置相机默认位置和相机高度，每次加载都从该视角进行飞行
        //参考：https://blog.51cto.com/u_15349906/5089049
        Cesium.Camera.DEFAULT_VIEW_RECTANGLE = Cesium.Rectangle.fromDegrees(80, 26, 150.0, 35.5);
        //一个标量，用于乘以相机位置并在设置相机以查看矩形后将其添加回来。零值意味着相机将查看整个 Camera#DEFAULT_VIEW_RECTANGLE ，大于零的值将使其远离范围，小于零的值将使其靠近范围。
        ...
        viewer = new Cesium.Viewer('sceneViewer', {
          ...
          fullscreenButton: false, //是否全屏显示，按钮显示出来后点击无反应
      ...
      const loadArcGisImageLayer = () => {
        //默认有一个bing的卫星图图层
        // console.log('loadArcGisImageLayer imageryLayers=' + viewer.imageryLayers.length);
        //不加载bing卫星图
      ...
      onMounted(() => {
        console.log('ymStationsMap.vue onMounted');
        ...
        screenSpaceEventHandler.setInputAction(function (movement) {
          ...
          if (cartesian) {
            ...
            let height = Math.ceil(viewer.camera.positionCartographic.height);
            //根据相机高度计算层级
            position.zoom = heightToZoom(height);
          ...
          if (scene.mode !== Cesium.SceneMode.MORPHING) {
            ...
            if (
              ...
              var currentPickObj = {};
              currentPickObj.id = pickedObject.id._id;
              currentPickObj.name = pickedObject.id._name;
              currentPickObj.content = pickedObject.id._content;
              currentPickObj.type = pickedObject.id._type;

              setMouseMovePickObject(currentPickObj);
          ...
          //如果未找到鼠标移动选中对象，设置选中对象为空
          if (!foundEntityInPosition) setMouseMovePickObject(null);
      ...
      onUnmounted(() => {
        console.log('ymStationsMap.vue onUnmounted');
        //清空变量
        viewer = null;
      });
      return {
